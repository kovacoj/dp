...
- `randmbc_synthetic_validation.R`
  Synthetic latent-data validation for issue `#20`: compares full covariance transport against randomized covariance transport in double and simulated single precision. Writes machine-readable outputs under `results/` and figures under `figures/`.
...
The focused covariance-transport validation writes its reproducible tables under `results/` and its figures under `figures/`.
