...
\[
	Q_X = \operatorname{orth}(Y_X),
	\qquad
	Q_Y = \operatorname{orth}(Y_Y),
\]
and then builds
\[
...
\[
	Q = \operatorname{orth}([Y_X, Y_Y]),
\]
and approximates both operators in the same reduced coordinates:
\[
	C_X \approx Q B_X Q^\top,
	\qquad
	C_Y \approx Q B_Y Q^\top.
\]
...
\[
	\widehat A = Q B_X Q^\top + \lambda I,
	\qquad
	\widehat B = Q B_Y Q^\top + \lambda I
\]
in one common orthonormal basis $Q$, then the covariance perturbations still enter through $\Delta A$ and $\Delta B$, but they no longer carry an additional coordinate mismatch between the source and target reduced models.

\begin{proposition}[Why the shared basis is numerically preferable]
	Suppose $Q$ is an orthonormal basis such that
	\[
		\|(I-QQ^\top)C_X\|_2 \le \varepsilon_X,
		\qquad
		\|(I-QQ^\top)C_Y\|_2 \le \varepsilon_Y,
	\]
	and define
	\[
		\widehat A = Q B_X Q^\top + \lambda I,
	...
	\[
		\norm{T_\lambda - \widehat T_\lambda}_2
		\lesssim
		K_\lambda(C_X,C_Y)(\varepsilon_X + \varepsilon_Y),
	\]
	but now the two covariance errors are measured in compatible reduced coordinates.
\end{proposition}

\begin{proof}[Proof sketch]
	The compression errors satisfy
	\[
		\norm{C_X - Q B_X Q^\top}_2 \le \varepsilon_X,
		\qquad
		\norm{C_Y - Q B_Y Q^\top}_2 \le \varepsilon_Y,
	\]
	so the regularized perturbations obey $\norm{\Delta A}_2 \le \varepsilon_X$ and $\norm{\Delta B}_2 \le \varepsilon_Y$. The transport bound then follows from the same square-root and inverse-square-root perturbation argument used above. The point is not a different asymptotic formula, but the cleaner geometry: both regularized matrix functions are built from one reduced coordinate system, so one avoids comparing source and target transports assembled in misaligned subspaces.
\end{proof}
...
\begin{enumerate}
	...
	\item shared-basis randomized covariance transport, with one reduced basis used for both covariance operators.
\end{enumerate}
...
Table~\ref{tab:joint-basis-summary} gives one compact representative slice of the benchmark. The numbers are averages across the four synthetic spectrum families at rank $20$, oversampling $5$, regularization $\lambda = 10^{-6}$, and double precision. The key fact is immediate: the shared basis leaves the covariance approximation error in the same broad range, but it collapses the transport and downstream future-sample errors by two to three orders of magnitude.

\begin{table}[t]
	\centering
	\caption{Representative benchmark slice showing the benefit of the shared basis. Averages are taken over the four synthetic spectrum families for rank $20$, oversampling $5$, $\lambda = 10^{-6}$, and double precision.}
	\label{tab:joint-basis-summary}
	\begin{tabular}{lccc}
		\toprule
