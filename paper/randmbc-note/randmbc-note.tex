...
The local R package \texttt{randMBC} implements precisely the reduced workflow described above: latent Gaussianization, randomized covariance approximation, regularized covariance transport, and the associated diagnostics. The first focused validation step is the script
\texttt{experiments/randmbc\_synthetic\_validation.R}, which compares two
families of methods on controlled latent data:
\begin{enumerate}
	\item a deterministic full covariance-transport baseline,
	\item randomized covariance transport at several ranks, one fixed oversampling level, three regularization levels, and two arithmetic regimes (double and simulated single precision).
\end{enumerate}

The synthetic data are intentionally operator-centered rather than climate-realistic. They are generated directly in latent space with decaying covariance spectra, block dependence, and separate source, target, and future samples. This design avoids confounding the first validation step with the full MBC pipeline and makes the operator-level diagnostics directly interpretable.
...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.9\textwidth]{../../figures/randmbc_cov_error_vs_rank.pdf}
...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.9\textwidth]{../../figures/randmbc_transport_error_vs_rank.pdf}
...
The first message of the focused validation is positive: the randomized covariance approximation behaves in the expected direction. Averaged over the seeds, the covariance error decreases from about \(0.40\) at rank \(5\) to about \(0.13\) at rank \(40\), with double and simulated single precision remaining close throughout. This is exactly the regime in which one should expect truncation error to dominate arithmetic error.

The second message is that regularization is not a cosmetic parameter. At rank \(40\), the mean transport error remains in the hundreds for \(\lambda = 10^{-6}\), drops to the thirties for \(\lambda = 10^{-4}\), and falls to roughly \(2.3\) for \(\lambda = 10^{-2}\). The regularized minimum eigenvalue makes the same point from the opposite direction: once the spectral gap created by \(\lambda\) is substantial enough, the inverse-square-root stage no longer amplifies covariance perturbations catastrophically.

The third message is still cautionary. Even in the best current regime, the downstream Pearson and Spearman dependence errors remain far from negligible. The present randomized prototype is therefore not yet application-ready. What the benchmark does establish is a disciplined numerical path: first drive the covariance and transport diagnostics into a favorable regime, then revisit climate-facing dependence correction once the operator-level approximation is genuinely under control.

\section{Conclusion and Next Steps}

The present note does not support the claim that randomized covariance transport is already a superior dependence-correction layer for multivariate bias correction. On the current controlled benchmark, the answer to the practical question ``did \texttt{randMBC} help?'' is: not yet. The deterministic full covariance-transport baseline remains markedly more accurate than the current low-rank randomized prototype.
