...
Throughout the thesis, uppercase Latin letters such as \(A\), \(S\), and \(Q\) denote matrices, but they are often viewed as concrete representations of linear maps between finite-dimensional inner-product spaces. Vectors are denoted by lowercase letters such as \(x\) and \(b\). For a least-squares problem we typically write \(A \in \R^{n \times d}\) with \(n \ge d\), understanding \(A\) as the matrix of a linear map from \(\R^d\) into \(\R^n\), and we consider the overdetermined problem
\[
...
Gaussian sketches, structured sketches, and mixed-precision variants are introduced more carefully in later chapters, but the notation fixed here remains standard throughout the text. In particular, the manuscript moves freely between matrix language and the corresponding operator viewpoint whenever that shift clarifies the numerical question under discussion.
