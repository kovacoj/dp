...
\]
where \(\lambda > 0\) is a regularization parameter.  In the language of optimal transport, this is a \emph{regularized whitening--coloring covariance transport}: the map first whitens the source covariance (inverse square root) and then colors the result with the target covariance (square root).  It coincides with the Bures--Wasserstein optimal-transport map when \(C_X\) and \(C_Y\) commute, but in general it is not the symmetric optimal-transport map.  The present thesis adopts it primarily for its algebraic tractability and its natural connection to matrix-function perturbation theory, not as a claim of optimality in the transport sense.
...
\begin{proposition}[Randomized covariance approximation]
	...
	parameter \(q\). Then the range-finder compression error
	\[
		\|(I-QQ^\top)C\|_2
	\]
...
It is important to distinguish the range-finder residual from the actual
surrogate error. The range finder controls
\(\|(I-QQ^\top)C\|_2\), which measures how well the column space of \(Q\)
captures the action of \(C\). The low-rank surrogate actually used in the
transport map is \(\widehat C = QBQ^\top\) with \(B = Q^\top C Q\), and its
...
\[
	...
	\|C - QQ^\top C QQ^\top\|_2.
\]
For a symmetric positive semidefinite matrix, these two quantities are related
but not identical: the range residual upper-bounds the surrogate error because
\(\|C - QQ^\top C QQ^\top\|_2 \le \|(I-QQ^\top)C\|_2 + \|CQQ^\top - QQ^\top C QQ^\top\|_2\),
and the second term vanishes when \(C\) commutes with the projection \(QQ^\top\).
In the experiments, the recorded covariance error is the surrogate error
\(\|C - \widehat C\|_2\), while the theory provides bounds on the range-finder
residual. The gap between them is typically small for SPD matrices with rapid
spectral decay.
...
\section{Independent and Shared Reduced Bases}
...
\[
	C_X \approx Q_X B_X Q_X^\top, \qquad C_Y \approx Q_Y B_Y Q_Y^\top,
\]
...
\[
	\widehat T_\lambda
	=
	(Q_X B_X Q_X^\top + \lambda I)^{-1/2}
	(Q_Y B_Y Q_Y^\top + \lambda I)^{1/2}.
\]
...
An alternative is the \emph{shared-basis} variant. Instead of computing two separate range finders, one constructs a single orthonormal basis \(Q\) that approximately spans the union of the dominant ranges of \(C_X\) and \(C_Y\). A natural construction is to form the combined sketch matrix
\[
	Y_{\mathrm{joint}} = [C_X\Omega,\; C_Y\Omega]
\]
and compute \(Q\) as an orthonormal basis for the range of \(Y_{\mathrm{joint}}\). Both covariance operators are then projected onto this shared basis:
\[
	C_X \approx Q B_X Q^\top, \qquad C_Y \approx Q B_Y Q^\top,
\]
with \(B_X = Q^\top C_X Q\) and \(B_Y = Q^\top C_Y Q\). The regularized transport map becomes
\[
	\widehat T_\lambda
	=
	(Q B_X Q^\top + \lambda I)^{-1/2}
	(Q B_Y Q^\top + \lambda I)^{1/2},
\]
and both matrix functions are now evaluated in the same reduced coordinate system. This eliminates the coordinate mismatch and is expected to reduce \(\|T_\lambda - \widehat T_\lambda\|_2\) even when the raw covariance errors \(\|C_X - \widehat C_X\|_2\) and \(\|C_Y - \widehat C_Y\|_2\) are comparable to those of the independent construction.

The shared-basis variant should not be understood as universally superior. Its advantage is a hypothesis to be tested experimentally: in regimes where the low-rank approximation is already reasonably accurate, shared coordinates may stabilize the transport map enough to make precision effects visible. In regimes where the approximation itself is too coarse, neither coordinate alignment nor precision choice will rescue the method. The experiments in \Cref{ch:experiments} test this hypothesis explicitly.
...
\begin{proposition}[Covariance sketch product perturbation]
	\label{prop:cov_sketch_perturbation}
	Let \(Z \in \R^{n \times p}\) be a centered data matrix with empirical
	covariance \(C = \frac{1}{n}Z^\top Z\), let
	\(\Omega \in \R^{p \times \ell}\) be a sketching matrix, and let the
	computed sample product be formed in working precision with unit roundoff
	\(u\). Write
	\[
		\widehat Y
		= \operatorname{fl}_u\!\left(\frac{1}{n}Z^\top(Z\Omega)\right)
		= C\Omega + \Delta Y.
	\]
	Then, under the standard inner-product rounding model with accumulation
	in precision \(u\),
	\[
		\|\Delta Y\|_2
		\le
		\frac{n u}{1 - n u}\;\frac{\|Z\|_2^2}{n}\;\|\Omega\|_2,
	\]
	provided that \(n u < 1\). In particular, writing
	\(\eta_{\mathrm{fp}} = \|\Delta Y\|_2 / \|\Omega\|_2\), one obtains
	...
	\[
		...
		u\,n\,\lambda_{\max}(C),
	\]
	so that the floating-point perturbation scales with the unit roundoff, the
	sample size entering the accumulation, and the spectral scale of the
	empirical covariance.

	\medskip
	\noindent\textit{Remark.} This bound models the combined product
	\(Z^\top(Z\Omega)/n\) as a single inner-product accumulation of length
	\(n\). In practice, the computation involves two stages --- first
	\(W = Z\Omega\) (accumulation over \(p\) terms per entry), then
	\(Y = Z^\top W/n\) (accumulation over \(n\) terms per entry) --- each with
	its own rounding contribution. A fully detailed two-stage bound would
	involve separate terms \(\gamma_p^{(u)}\) and \(\gamma_n^{(u)}\) for the
	respective accumulations. The present bound is a simplified first-order
	model that subsumes both stages into the dominant \(\gamma_n\) factor; the
	implementation provides an optional two-stage variant in
	\texttt{randMBC::sketch\_forward\_bound} with \texttt{two\_stage = TRUE}.
\end{proposition}

\begin{proof}
	This is a direct application of the standard matrix-product rounding model
	\cite[Thm.~3.5]{higham2022mixed}. Each column of
	\(\frac{1}{n}Z^\top(Z\Omega)\) is an inner product of length \(n\),
	so the columnwise relative perturbation is bounded by \(\gamma_n = nu/(1-nu)\).
	Taking norms and using \(\|C\|_2 = \|Z\|_2^2/n\) gives the result.
\end{proof}

The practical content of \Cref{prop:cov_sketch_perturbation} is that the floating-point contribution to the covariance sketch perturbation is computable from the data norms, the accumulation length, and the unit roundoff alone. The balanced regime
\[
	\eta_{\mathrm{fp}} \lesssim \eta_{\mathrm{rand}}
\]
can therefore be tested before committing to a precision choice, by comparing \(\eta_{\mathrm{fp}}\) against a randomized truncation error estimate (e.g.\ from a range-finder residual).
...
\begin{proof}[Proof sketch]
	These bounds follow from the Fr\'echet-derivative structure of matrix
	square-root and inverse-square-root functions on the positive definite cone.
	For the square root, the Fr\'echet derivative \(L_f(A, E)\) of the map
	\(f(A) = A^{1/2}\) satisfies the Sylvester equation
	\(A^{1/2} L_f + L_f A^{1/2} = E\); bounding \(\|L_f\|_2\) via the
	spectral norm of the Sylvester operator yields the first inequality with the
	constant \((2\beta_0^{1/2})^{-1}\). The inverse-square-root bound then
	...
	\]
	This algebraic identity is useful but does not by itself constitute a
	perturbation bound; the rigorous bound follows from matrix-function
	perturbation theory \cite[Chs.~3 and 6]{higham2008functions}.
	Combined with the square-root perturbation bound and the norm estimates
	\(\|A^{-1/2}\|_2 \le \alpha_0^{-1/2}\),
	\(\|\widehat A^{-1/2}\|_2 \le \alpha_0^{-1/2}\).
	For a rigorous treatment of matrix-function Fr\'echet derivatives and
	perturbation bounds, see \cite[Chs.~3 and 6]{higham2008functions}. The
	important qualitative point is the scaling: the inverse square root is more
	sensitive near small eigenvalues, with amplification proportional to
	\(\alpha_0^{-3/2}\).
\end{proof}

\begin{theorem}[Perturbation of covariance transport --- first-order schematic bound]
	\label{thm:cov_transport_perturbation}
	...
	\[
		K_\lambda(C_X,C_Y,\Delta A,\Delta B)
		=
		\max\!\left\{
		\frac{\|A^{-1/2}\|_2}{2\beta_0^{1/2}},\;
		\frac{\|\widehat B^{1/2}\|_2}{2\alpha_0^{3/2}}
		\right\},
	\]
	so that
	\[
		\|T_\lambda - \widehat T_\lambda\|_2
		\le
		K_\lambda\,(\eta_A + \eta_B).
	\]
	This is a first-order schematic bound used to guide experimental design.
	The constant in this bound is a schematic first-order amplification factor;
	it has not been tracked to a fully rigorous constant under the stated
	hypotheses. See the implementation's two-stage forward model for the
	numerically active version.
	When \(\|\Delta A\|_2\) and \(\|\Delta B\|_2\) are small relative to the
	regularized spectral gaps, the amplification factor is essentially a constant
	depending on \(\lambda_{\min}(C_X+\lambda I)\) and
	\(\lambda_{\min}(C_Y+\lambda I)\). In particular, the inverse-square-root
	term deteriorates at the rate
	\(\lambda_{\min}(C_X+\lambda I)^{-3/2}\). A fully rigorous bound with
	sharper constants would require tracking higher-order terms and exact
	Sylvester-operator norms; the present statement suffices to explain the
	qualitative regime structure observed in the experiments.
\end{theorem}
...
This formulation leads to a concrete algorithmic decomposition, presented formally as \Cref{alg:mp_rand_cov_transport}.

\begin{algorithm}[t]
	\caption{Mixed-precision randomized covariance transport}
	\label{alg:mp_rand_cov_transport}
	\KwIn{Historical model data \(X_h\), future model data \(X_f\), historical reference data \(Y_h\), target rank \(r\), oversampling \(q\), regularization \(\lambda\), sketch precision \(u_\ell\)}
	\KwOut{Corrected future sample \(\widetilde X_f^{\mathrm{corr}}\), diagnostics}
	\BlankLine
	Apply QDM-type marginal correction to obtain \(\widetilde X_h\), \(\widetilde X_f\)\;
	Transform \(\widetilde X_h\), \(\widetilde X_f\), \(Y_h\) to latent rank-normal variables \(Z_X\), \(Z_f\), \(Z_Y\)\;
	\texttt{// Low-precision block:}\;
	Form randomized covariance sketches in precision \(u_\ell\) (simulated or hardware-native):
	\(Y_X = \tfrac{1}{n}Z_X^\top(Z_X\Omega_X)\), \quad
	\(Y_Y = \tfrac{1}{n}Z_Y^\top(Z_Y\Omega_Y)\)\;
	\texttt{// High-precision block:}\;
	Compute orthonormal bases \(Q_X\), \(Q_Y\) (or shared basis \(Q\)) via QR in double precision\;
	Form small covariance cores \(B_X = Q_X^\top C_X Q_X\), \(B_Y = Q_Y^\top C_Y Q_Y\) in double precision\;
	Build regularized low-rank covariance surrogates \(\widehat C_{X,\lambda}\), \(\widehat C_{Y,\lambda}\)\;
	Evaluate regularized transport map in double precision:
	\(\widehat T_\lambda = (\widehat C_{X,\lambda})^{-1/2}\,(\widehat C_{Y,\lambda})^{1/2}\)\;
	Apply \(Z_f^{\mathrm{corr}} = Z_f\,\widehat T_\lambda\)\;
	Restore QDM marginals by rank rearrangement\;
	\Return corrected future sample and diagnostics\;
	\BlankLine
	\textbf{Diagnostics:} covariance error \(\|C - \widehat C\|_2\); transport-map error \(\|T_\lambda - \widehat T_\lambda\|_2\); induced future-data perturbation \(\|Z_f T_\lambda - Z_f \widehat T_\lambda\|_F\); ratio \(\eta_{\mathrm{fp}}/\eta_{\mathrm{rand}}\); runtime\;
\end{algorithm}
