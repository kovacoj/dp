...
To express dependence in a form closer to linear algebra, it is convenient to pass to a latent Gaussianized space. For each column \(j\), let \(\widehat F_j\) denote the empirical distribution function of the corresponding marginally corrected calibration sample, and let \(\Phi\) denote the standard normal distribution function. We then define
\[
	Z_X(:,j) = \Phi^{-1}\!\bigl(\widehat F_j(\widetilde X_h(:,j))\bigr),
	\qquad
	Z_Y(:,j) = \Phi^{-1}\!\bigl(\widehat F_j(Y_h(:,j))\bigr),
\]
and similarly obtain \(Z_f\) from \(\widetilde X_f\). In this representation, marginal correction is separated from dependence correction: the marginals are handled before and after the latent transform, while the multivariate adjustment acts on the standardized latent variables.
...
\]
where \(E_{\mathrm{rand}}\) denotes randomized approximation error and \(E_{\mathrm{fp}}\) floating-point error. The key regime is the balanced one,
\[
...
\[
	...
	\|\Delta B\|_2 \le \eta_B,
\]
then one expects matrix-function perturbation estimates to lead to a bound of the form
\[
	\|T_\lambda - \widehat T_\lambda\|_2
	\lesssim
	K_\lambda(C_X,C_Y)(\eta_A + \eta_B),
\]
where the amplification factor \(K_\lambda\) deteriorates as \(\lambda_{\min}(C_X + \lambda I)\) or \(\lambda_{\min}(C_Y + \lambda I)\) approaches zero. This is the mathematical reason why regularization is not a secondary implementation detail in the present problem. It is part of the perturbation theory itself.
...
\]
For spectra with visible decay, the randomized term is governed by the tail beyond the chosen rank, up to oversampling constants \cite{halko2011finding,martinsson2020randnla}. The floating-point term, by contrast, is controlled by the precision used in the sketch products and by the accumulation length. The thesis question is precisely how these two error sources should be balanced against each other.
